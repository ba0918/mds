...
| フィールド行 | 行頭の空白を除いて `- 名前: 値` の形の行 | docs/spec/mds.md#用語 |
| 箇条書き | 行頭の空白を除いて `- ` で始まり `名前: 値` の形でない一覧行 | docs/spec/mds.md#用語 |
| 文 | 見出しや前置部の下にある、一覧でも表でもない空でない行。リスト項目の継続段落は含まない | docs/spec/mds.md#用語 |
